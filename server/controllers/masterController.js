...
import {
    errorFormat,
    successFormat
} from '../middleware/formatResponse.js'
...
export const getAddUpdateLocation = expressAsyncHandler(async (req, res) => {
    try {
        const ClusterData = await db.sequelize.query(`Call SP_Location(:_User_ID,:_Is_Admin,:_Mode,:_Location_Id,:_Cluster_Id,:_SubCluster_Id,:_Vertical_Id,:_Location_Name,:_Location_Status)`, {
            replacements: {
                _User_ID: req.body.User_ID ? req.body.User_ID : '',
                _Is_Admin: req.body.Is_Admin ? req.body.Is_Admin : '',
                _Mode: req.body.Mode ? req.body.Mode : '',
                //Mode- GET (Get all Location)
                //Mode - INSERTUPDATE (add or update the Location )
                _Location_Id: req.body._Location_Id ? req.body._Location_Id : 0,
                _Cluster_Id: req.body.Cluster_Id ? req.body.Cluster_Id : 0,
                _SubCluster_Id: req.body.SubCluster_Id ? req.body.SubCluster_Id : 0,
                _Vertical_Id: req.body.Vertical_Id ? req.body.Vertical_Id : 0,
                _Location_Name: req.body.Location_Name ? req.body.Location_Name : '',
                _Location_Status: req.body.Location_Status ? req.body.Location_Status : ''
            }
        });
        res.status(200).send(successFormat("Success", "Success", ClusterData, []));
    } catch (error) {
